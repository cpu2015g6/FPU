...
//for precise operation fmul_s
extern int encode(uint32_t a);

uint32_t mulround_s(uint32_t ulp, uint32_t f){
  if(f >> 31){
    if(f << 1)
      return 1;
    else
...
uint32_t fmulsub_s(uint32_t a, uint32_t b){
  ...
  return (sgn << 31) + (exp << 23) + man + mulround_s((man%2),frac);
}
